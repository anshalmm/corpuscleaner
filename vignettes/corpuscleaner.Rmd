...
And functions relative to `Frequency Lists`: 
...
Here are some examples of basic Text Analysis of the datasets using the `novels()` and `document_by_ID` functions.
...
# Unigram Analysis
Documents
Text = document_by_ID(id == 7) 
UAnalysis = unnest_unigrams(Text)
UWords = UAnalysis %>%
  anti_join(get_stopwords("en", source = "smart"))
UWords
...
# Bigram Analysis
WHText = document_by_ID(id == 9)
BAnalysis = unnest_bigrams(WHText)
BWords = BAnalysis %>%
